...
/// [Utils] class contains utility methods for `intl_phone_number_countries` library
class Utils {
  ...
  static List<Country> filterCountries({
    ...
    if (value.isNotEmpty) {
      return countries
          .where(
            (Country country) => country.alpha3Code!.toLowerCase().startsWith(value.toLowerCase()) || country.name!.toLowerCase().contains(value.toLowerCase()) || Utils.getCountryName(country, locale)!.toLowerCase().contains(value.toLowerCase()) || country.dialCode!.contains(value.toLowerCase()),
          )
          .toList();
    }
